...
function VoiceConnection:_play(stream, duration)
	...
	while elapsed < duration do
		...
		if not self._playing then stream:close() print(self._playing) break end
...
function VoiceConnection:playYoutube(url)
	...
	local stream = YoutubeProcess(url, SAMPLE_RATE, CHANNELS,self._client)
	self:_play(stream, duration)
	return stream:close()
